...
class Avatar extends Phaser.GameObjects.Container {
  constructor(scene, p) {
    ...
    this.setSize(120, 120); scene.add.existing(this);

    const g = scene.add.graphics();
    g.lineStyle(3, 0x111111, 1).strokeCircle(0, 0, 48); this.add(g);
    ...
    this.add(this.num); this.add(this.name);

    const phSize = 70;
    this.photo = scene.add.image(0, 14, "__ph__").setDisplaySize(phSize, phSize);
    const mG = scene.add.graphics().fillStyle(0xffffff,1).fillCircle(0,14,34);
    const mask = mG.createGeometryMask(); this.photo.setMask(mask);
    this.add(this.photo); this.add(mG);
    this.add(scene.add.graphics().lineStyle(2,0x111111,1).strokeCircle(0,14,34));
...
class HostScene extends Phaser.Scene {
  ...
  syncAvatars(){
    for (const [id,p] of players.entries()){
      ...
      if (!a){ a = new Avatar(this, p); this.avatars.set(id,a); }
      else { a.updateFrom(p); }
    }
